...
class UploadService:
    ...
    async def simply_upload_video(self, video_id: int, file: UploadFile):
        upload_dir = "uploads"
        ...
        try:
            ...
            # 중간 지점에서 썸네일 추출
            (
                ffmpeg.input(file_path, ss=duration / 2)
        ...
        self.repository.update_video(
            video_id=video_id, file_path=file_path, transcription_status="SUCCESS"
        )
        self.db.commit()
    ...
    async def process_transcript_upload(
        ...
        # 메타데이터 저장
        video = self.repository.save_video_metadata(
            title=title, file_path=file_path, user_id=user_id
        )
        background_tasks.add_task(
    ...
    def _process_video_transcript(self, video_id: int, file_path: str):
        ...
        try:
            ...
            try:
                ...
                self.repository.update_video(
                    video_id=video_id, transcription_status="SUCCESS"
                )
                self.db.commit()
                logger.info(log_msg)
                background_tasks_status[video_id]["log_messages"].append(log_msg)
                ...
                self.db.rollback()
                self.repository.update_video(
                    video_id=video_id, transcription_status="FAILED"
                )
                self.db.commit()
    ...
    async def upload_video(self, file: UploadFile, title: str, user_id: int) -> Video:
        ...
        # DB에 비디오 정보 저장
        db_video = Video(title=title, file_path=file_path, user_id=user_id)
